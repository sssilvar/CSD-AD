...
if __name__ == "__main__":
    ...
    # Load ROI names
    print('[  INFO  ] Loading ROIs...')
    regions_file = os.path.join(root, 'param', 'FreeSurferColorLUT.csv')
    df_regions = pd.read_csv(regions_file, index_col=['region_id'])


    for roi in df['ROI'].cat.categories:
        roi_name = df_regions.loc[roi, 'label_name']
        print('[  INFO  ] Classifying %s' % roi_name)
        df_roi = df[df['ROI'] == roi].drop('ROI', axis=1)
        
        # Standardize and correct 
        df_std = get_features_and_labels(df_roi)
        ...
        clf_grid = GridSearchCV(svm, param_grid, cv=StratifiedKFold(n_splits=10), iid=False)
        ...
        plt.title('ROC for %s' % roi_name)

        fig_file = join(dirname(data_file), 'ROC', basename(data_file)[:-4] + '_roc_%d.png' % roi)
